...
class OrderModal {
    ...
    open() {
        console.log('📂 Opening Square ordering in new tab...');

        // Open Square in new window/tab (avoids iframe restrictions)
        window.open(this.squareURL, '_blank', 'noopener,noreferrer');

        // Track event (optional - for analytics)
        if (typeof gtag !== 'undefined') {
            gtag('event', 'open_square_ordering', {
                event_category: 'engagement',
                event_label: 'Square Ordering Opened'
            });
        }
    }
